// Namespace
resource "kubernetes_namespace_v1" "goclaw" {
  metadata {
    name = var.namespace
    labels = {
      name        = var.namespace
...
resource "kubernetes_config_map_v1" "goclaw_config" {
  metadata {
    name      = "goclaw-config"
    namespace = kubernetes_namespace_v1.goclaw.metadata[0].name
  }
  data = {
...
resource "kubernetes_secret_v1" "goclaw_secrets" {
  ...
  data = {
    ...
    "postgres-dsn"               = base64encode("postgresql://goclaw:${var.postgres_password}@postgres:5432/goclaw?sslmode=disable")
...
// PostgreSQL StatefulSet (in-cluster only)
    ...
    template {
      ...
      spec {
        init_container {
          ...
          image   = "postgres:14"
          command = ["/bin/sh", "-c", "until pg_isready -h postgres -p 5432 -U goclaw; do echo Waiting; sleep 2; done;"]
        }
        init_container {
          name    = "run-migrations"
        ...
        container {
          ...
          image = "${var.goclaw_image}:${var.goclaw_version}"
          port {
            container_port = var.goclaw_port
            name           = "gateway"
          }
          env {
            name  = "GOCLAW_PORT"
  ...
  spec {
    ...
    job_template {
      ...
      spec {
        template {
          metadata {
            labels = { app = "goclaw-backup" }
          }
          spec {
            container {
              name    = "backup"
              image   = "${var.goclaw_image}:${var.goclaw_version}"
              command = ["/usr/local/bin/goclaw", "backup", "-o", "/backups/goclaw-$(date +%Y%m%d%HP%SM).tar.gz"]
              env {
                name = "GOCLAW_POSTGRES_DSN"
                value_from {
                  secret_key_ref {
